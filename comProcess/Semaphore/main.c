...
//	int product = 1;
	...
	int shmid = shmget(60, 4, IPC_CREAT | 0600);
	int* product = (int*)shmat(shmid, NULL, 0);
	...
	*product = 0;
	...
	if (0 == (p1 = fork())) // p1子进程
	{
		while (1)
		{
			...
			// printf("2\n");
			(*product)++;
			printf("Producer  %d: +1 products %d\n", getpid(), *product);
	...
	else // p1父进程
	{
		if (0 == (p2 = fork())) // p2子进程
		{
			while (1)
			{
				...
				// printf("4\n");
				(*product)--;
				printf("Consumer1 %d: -1 products %d\n", getpid(), *product);
				semaphore_v(semid, 0); // v(mutex)
		...
		else // p2 & p1 父进程
		{
			while (1)
			{
				...
				// printf("6\n");
				(*product)--;
				printf("Consumer2 %d: -1 products %d\n", getpid(), *product);
				semaphore_v(semid, 0); // v(mutex)
				semaphore_v(semid, 1); // v(empty)
				sleep(5);
			}
